...
export default function ClearAllModal({ phaseName, planName, onClearPhase, onClearPlan, onClearAll, onClearActions, onCancel }: Props) {
  const { language } = useStore();
  const [confirmingActions, setConfirmingActions] = useState(false);
  const [confirmInput, setConfirmInput] = useState('');

  function handleClearActionsClick() {
      ...
      <div className="encounter-dialog" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
        ...
        {!confirmingActions ? (
          <>
            ...
            <div style={SCOPE_BTNS}>
              ...
              <button
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#0c1a2e'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; }}
          ...
          <>
            ...
            <p style={{ ...WARNING_STYLE, marginBottom: 8 }}>{t('clearActionsPrompt', language)}</p>
            <input
            ...
            />
            <div className="encounter-dialog-actions">
              <button className="encounter-dialog-cancel" onClick={handleCancelConfirm}>{t('clearActionsBack', language)}</button>
              <button
                className="encounter-dialog-confirm"
                style={{ background: confirmInput === planName ? '#1d4ed8' : '#1e3a5f', cursor: confirmInput === planName ? 'pointer' : 'not-allowed', opacity: confirmInput === planName ? 1 : 0.5 }}
                disabled={confirmInput !== planName}
                onClick={handleConfirmDelete}
              >
                {t('clearActionsConfirm', language)}
              </button>
            </div>
          </>
        )}
